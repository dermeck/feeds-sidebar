...
export const readOpmlFile = async (file: File): Promise<ParseOPMLResult> => {
...
// TODO extract to opmlProcessor
const parseXml = (fileContent: string): ParseOPMLResult => {
    const options = {
    ...
    if (parsed.opml !== undefined && parsed.opml.head !== undefined && parsed.opml.head.ownerId === exportedOwnerId) {
        ...
        console.log('parsedFolders', parsedFolders);
        console.log('parsedFeeds', parsedFeeds);
